...
# TELL article tokens (Beyond Zork TELL-TOKENS): each prints an object's desc
# prefixed with its article.  The ``…O``/``…I`` variants take no argument and
# reprint for the implied object; the rest consume the next TELL argument.
_TELL_ARTICLE_NOARG = frozenset({"AO", "ANO", "CAO", "CANO", "THEO", "CTHEO", "THEI", "CTHEI"})
_TELL_ARTICLE_TOKENS = frozenset({"A", "AN", "CA", "CAN", "THE", "CTHE"}) | _TELL_ARTICLE_NOARG
...
class ZilTranslator:
    ...
    def _tell_segments(self, form: list) -> tuple[list[str], bool]:
        ...
        while i < len(parts):
            ...
            if isinstance(item, str) and item.upper() == "B":
                ...
                i += 1
                continue
            # Article tokens (Beyond Zork TELL-TOKENS): print an object's
            # desc prefixed with its article.  ``A``/``AN`` → "a/an" (lower),
            # ``CA``/``CAN`` → "A/An" (capitalised), ``THE``/``CTHE`` → "the/The".
            # Each consumes the next argument (the object); the ``article``
            # helper honours NARTICLE/VOWEL flags and returns a leading-space
            # segment.  The ``…O``/``…I`` no-arg variants reprint the article
            # for the implied object (``P-IT-OBJECT``, always a safe read).
            if isinstance(item, str) and not isinstance(item, Str) and item.upper() in _TELL_ARTICLE_TOKENS:
                # A bare atom (``Str`` would be a quoted literal like "a" or
                # "the" — those are real text, not article tokens).
                tok = item.upper()
                receiver = substrate_receiver("article")
                the = "True" if tok in ("THE", "CTHE", "THEO", "CTHEO", "THEI", "CTHEI") else "False"
                cap = "True" if tok.startswith("C") else "False"
                if tok in _TELL_ARTICLE_NOARG:
                    obj = "context.player.zstate_get('P-IT-OBJECT')"
                else:
                    i += 1
                    obj = self._translate_expr(parts[i]) if i < len(parts) else "None"
                segments.append(f"{receiver}.article({obj}, {the}, {cap})")
                i += 1
                continue
            # Plain string or expression
            expr = self._translate_expr(item)
            # A bare object atom in a TELL (ZIL's ``'OBJ`` quote form, e.g.
            # ``<TELL "glass " 'BCASE " near the ">``) must print the object's
            # NAME, not the raw Object — the segments are concatenated with
            # ``+``, so a bare Object raises "can only concatenate str (not
            # Object) to str" (SHOP-DOOR crashed entering the weapon shop).
            # The ``D ,OBJ`` path above already emits ``.desc()``; mirror it
            # for the bare/quoted object-atom form.
            if isinstance(item, str) and not isinstance(item, Str) and item.upper() in self.object_atoms:
                expr = f"{expr}.desc()"
    ...
    def _translate_prop_name(self, node: Any) -> str:
        ...
        A *computed* property — where the prop arg is a routine variable or a
        form rather than a ``P?`` atom literal — translates to a runtime
        expression instead of a string literal; ``getp`` resolves the value
        (a ``P?`` property number from ``PDIR-LIST`` or a direction loop var)
        back to the exit property.  Two cases:

        * ``<GETP ,HERE .DIR>`` — ``.DIR`` is a routine param/aux var holding a
          ``P?`` number; emit the bare local (mirrors ``_translate_flag_name``).
        * ``<GETP .RM <GETB ,PDIR-LIST .DIR>>`` — the prop arg is a form; emit
          the translated expression.

        :param node: Property-atom AST node.
        :returns: A Python string-literal expression (or a runtime expression).
        """
        if isinstance(node, list) and node:
            if len(node) == 1:
                node = node[0]
        ...
        if isinstance(node, str):
            # `.X` deref of a routine param/aux var — emit the local name so
            # ``getp`` receives the variable's runtime value (a P? number).
            if node.startswith("."):
                bare = node[1:]
                local_atoms = {p.upper() for p in self.routine.params + self.routine.aux_vars}
                if bare.upper() in local_atoms:
