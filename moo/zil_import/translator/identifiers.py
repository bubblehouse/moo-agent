...
def register_substrate_overrides(overrides: dict[str, str]) -> None:
    ...
    Actor, but ``moo/zil_import/verbs/actor/score.py``
...
def atom_to_snake(atom: str) -> str:
    """
    Convert a ZIL atom (UPPER-KEBAB, ``?``, ``!`` allowed) to a snake_case
    identifier suitable for use as a verb-file stem or Python attribute.

    Centralises the three-replace chain that previously appeared at ~30
    call sites across the translator and generator.  Predicates encode
    ``?`` as ``_p`` and macro markers encode ``!`` as ``_b``.

    :param atom: ZIL atom such as ``"FOREST-ROOM"`` or ``"EQUAL?"``.
    :returns: ``"forest_room"`` / ``"equal_p"``.
    """
    return atom.lower().replace("-", "_").replace("?", "_p").replace("!", "_b")
