#!moo verb put apply stuff insert place hide --on "Thing" --dspec this
...
if _.thing.invoke_verb("pre_put"):
    return
...
if ((_.thing.weight(prsi) + _.thing.weight(prso)) - prsi.getp("size")) > prsi.getp("capacity"):
    print("There's no room.")
    return

if not _.thing.is_held(prso) and prso.flag("trytakebit"):
    print("You don't have the " + prso.desc() + ".")
    return True

if not _.thing.is_held(prso) and not _.thing.itake():
    return True

prso.moveto(prsi)
prso.set_flag("touchbit", True)
# SCORE-OBJ is Zork-only (HHG has no per-item value scoring); skip
# the call when the verb isn't registered on this game's Thing class
# rather than letting the bare ``_.thing.score_obj`` AttributeError.
if _.thing.has_verb("score_obj"):
