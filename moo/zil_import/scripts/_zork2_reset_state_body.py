# Reset Zork II world state to canonical opening positions.
#
# Two callers (mirrors the zork1 reset body):
#   1. moo_init --bootstrap zork2 [--sync] — generator.py copies this file
#      to moo/bootstrap/zork2/099_reset_state.py so it runs after rooms /
#      objects / exits are loaded.
#   2. moo/zil_import/scripts/zork2_smoke.py — reads this file's text and
...
# pollution) + park the Adventurer at the canonical start (Inside the
# Barrow) + basic zstate + the substrate property-grant loop.  Per-object
# canonical-position seeding (the long zork1 list) gets added here as
# shakedown surfaces the need.  Idempotent.  Hardcodes ``zork2.local``.
...
def _snapshot_capture(site, repo, snapshot_path):
    ...
    print(f"zork2 reset: captured snapshot to {snapshot_path} ({len(objects_data)} objects)")
...
def _snapshot_restore(snapshot_path, site):
    ...
    print(f"zork2 reset: restored snapshot from {snapshot_path}")


def _reset_zork2_world(site):
    ...
    ``zork2.local`` Site row, the dispatch below skips this call entirely.
    """
    ...
    snapshot_path = _SNAPSHOT_DIR / f"zork2-site-{site.pk}.json"
    try:
        if snapshot_path.exists():
            _snapshot_restore(snapshot_path, site)
        else:
            _snapshot_capture(site, "zork2", snapshot_path)
    except _SnapshotSiteMismatch as exc:
        print(f"zork2 reset: snapshot site mismatch, skipping restore: {exc}")
    except Exception as exc:  # pylint: disable=broad-except
        print(f"zork2 reset: snapshot {type(exc).__name__}: {exc}")

    # Gameplay runs through the non-wizard Adventurer avatar (real ACL
    # enforcement).  Zork II opens at Inside the Barrow (ZIL INSIDE-BARROW,
    # set by GO via <SETG HERE ,INSIDE-BARROW>).
    adventurer = Object.global_objects.get(name="Adventurer", site=site)
    start = Object.global_objects.get(name="Inside the Barrow", site=site)
    ...
    # Stop the Carousel Room spinning so its eight passages are
    # deterministic (CAROUSEL-ROOM-FCN randomizes the walk direction at
    # PROB 80 while CAROUSEL-FLIP-FLAG is unset).  Canonically the flag is
    # flipped by the *robot* pressing the triangular button (the Adventurer
    # pressing any button dies); seeding it here is the smoke-safe shortcut,
    # mirroring zork1 seeding MAGIC-FLAG/CYCLOPS-FLAG to reach gated content.
    # A future shakedown that wants to drive the real robot puzzle should
    # unset this seed.
    adventurer.set_property("zstate_carousel_flip_flag", True)
    ...
    # Force the two starting items onto the barrow floor with the lamp OFF —
    # the canonical opening.  The bootstrap's get_or_create doesn't reposition
    # objects that already exist, so a prior session's inventory (and a lit
    # lamp) survives into the captured snapshot; re-placing them every reset
    # makes the opener deterministic regardless of snapshot drift.
    for _item_name in ("lamp", "elvish sword"):
        _item = Object.global_objects.filter(name=_item_name, site=site).first()
        if _item:
            _item.location = start
            if _item_name == "lamp":
                _item.set_property("onbit", False)
            _item.save()
...
# zork2.local Site row may not yet exist — skip silently so the loader can
# proceed; the next --sync (after the site is created) runs the reset.
_site = Site.objects.filter(domain="zork2.local").first()
if _site is None:
    print("zork2 reset: site not yet created, skipping")
else:
    _reset_zork2_world(_site)
