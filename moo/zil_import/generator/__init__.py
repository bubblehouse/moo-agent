...
# XZIP (Beyond Zork) exit-table kind constants — the high-byte values from
# beyond.zil ``macros.zil`` (``<CONSTANT CONNECT #2 001000000000>`` = 0x0200,
# etc.).  Used by ``_gen_exits`` to build per-direction XTYPE word-tables.
_XZIP_NO_EXIT = 0x0100
_XZIP_CONNECT = 0x0200
_XZIP_SCONNECT = 0x0300
_XZIP_FCONNECT = 0x0400
_XZIP_DCONNECT = 0x0500
_XZIP_SORRY_EXIT = 0x0600
_XZIP_X_EXIT = 0x0700

# Generated-file header.  _SHORT drops the do-not-edit warning where context already carries it.
_GENERATED_HEADER = "# Generated by moo/zil_import — do not edit by hand"
_GENERATED_HEADER_SHORT = "# Generated by moo/zil_import"
...
# See ``moo/zil_import/game_config.py`` for the per-game override seam.
...
def _gen_rooms(rooms: dict[str, ZilRoom], display_names: dict[str, str], cfg: GameConfig) -> str:
    ...
    for atom, room in rooms.items():
        ...
        if room.action:
            ...
            # Phase 7: expose the ZIL ROUTINE atom so ``perform.py`` /
            # ``do_command`` can route M-Beg / M-End hooks through
            # :verb:`d_apply` instead of relying on the parser to find the
            # per-room ``preturnfunc`` / ``turnfunc`` verb by name.
            lines.append(f"{var}.set_property('action', {room.action.lower().replace('-', '_')!r})")
...
def _gen_objects(
    ...
    for atom, obj in objects.items():
        ...
        # Resolve location.  A well-formed location is a single container
        # atom (a str).  Guard against a non-atom slipping through (e.g. a
        # mis-parsed direction-exit value that came in as a list) — those are
        # unhashable and would crash the membership tests below.  Degrade to
        # "no location" rather than abort the whole regen.
        obj_location = obj.location if isinstance(obj.location, str) else None
        if obj.location is not None and obj_location is None:
            log.warning(
                "Object %s has non-atom location %r; treating as unplaced.",
                atom,
                obj.location,
            )
        if obj_location and obj_location in rooms:
            loc_expr = f"_rooms[{repr(obj_location)}]"
        elif obj_location and obj_location in objects:
            # Forward reference to another object — start with None and
            # patch after every object has been created.
            loc_expr = "None"
            deferred_locations.append((atom, obj_location))
        elif obj_location:
            # Unknown atom (e.g. a global pseudo-container).  Best-effort
            # lookup through the running ``_objects`` dict.
            loc_expr = f"_objects.get({repr(obj_location)})  # {obj_location}"
        else:
            loc_expr = "None"
...
def _gen_exits(
    ...
    exit_tables = bool(cfg and cfg.exit_tables)

    def _xzip_exit_table(ex: ZilExit) -> str | None:
        """Build a Beyond Zork XTYPE word-table literal for one exit.

        Layout (``macros.zil``): word[XTYPE]=kind|len, word[XROOM]=dest/str/fcn,
        word[XDATA]=aux.  Kind high-byte values are the v5 exit constants; a
        unit ``len`` (1) draws a one-cell path on the auto-map (SORRY carries no
        len).  Returns a Python list-literal string, or ``None`` if the dialect
        doesn't use tables.
        """
        if not exit_tables:
            return None
        if ex.per_routine:
            # FCONNECT: XROOM is the function; map draws it when len>0.
            return f"[{_XZIP_FCONNECT | 1}, {repr(ex.per_routine)}]"
        if ex.dest and ex.dest in rooms:
            dest_var = _room_atom_to_var(ex.dest)
            if ex.message:
                # SCONNECT: print XDATA message, then move to XROOM.
                return f"[{_XZIP_SCONNECT | 1}, {dest_var}, {_py_str(ex.message)}]"
            # CONNECT: unconditional move to XROOM.
            return f"[{_XZIP_CONNECT | 1}, {dest_var}]"
        if ex.message:
            # SORRY-EXIT: message lives at XROOM; stay put (no len).
            return f"[{_XZIP_SORRY_EXIT}, {_py_str(ex.message)}]"
        # Blocked / unresolved destination.
        return f"[{_XZIP_NO_EXIT}, 0]"
    ...
    for atom, room in rooms.items():
        ...
        for ex in room.exits:
            ...
            # XZIP: also store the direction as a Z-machine XTYPE word-table
            # property so Beyond Zork's own movement / MARK-EXITS / auto-map
            # read the exit via ``<GETP ,HERE ,P?dir>`` → ``getp("<dir>")``.
            _table = _xzip_exit_table(ex)
            if _table is not None:
                lines.append(f"{room_var}.set_property({repr(direction_lower)}, {_table})")
...
def _gen_globals(globals_dict: dict[str, object], cfg: GameConfig) -> str:
    ...
    if globals_dict:
        for name, value in sorted(globals_dict.items()):
            sanitized = re.sub(r"[^a-z0-9_]", "_", name.lower().replace("-", "_"))
...
def generate_all(
    ...
    *,
    ...
    typed_rules = ir.rules  # verb → list[ZilSyntaxRule], typed Phase 1 reification
    ...
    # Coverage audit: accumulates per-routine drops during emission and
    # writes ``{output_dir}/coverage.json`` at the end of regen.  See
    # ``moo/zil_import/audit.py`` for the drop catalog and the rationale.
    # Test ratchet lives at ``moo/zil_import/tests/test_translator_coverage.py``.
    audit = RegenAudit(game=cfg.dataset_name)
    ...
    _SKIP_ROUTINES = {
        ...
        "HELD?",
        # Hand-written: verbs/thing/helpers/d_apply.py.  Auto-translated body
        # emits Python 2's ``apply(fcn, foo)`` builtin which doesn't exist in
        # Python 3 — crashes ``perform.py`` the moment ``loc.action`` is set
        # (Phase 7).  Replacement does stage-aware dispatch via ``has_verb`` +
        # ``invoke_verb`` on the right receiver.
        "D-APPLY",
    ...
    def _filename_from_shebang(code: str) -> str:
        ...
        snake = atom_to_snake(first.replace("@", ""))
    ...
    def _emit_verb_clauses(translator: ZilTranslator, target: Path) -> set[str]:
        ...
        if has_overlap:
            ...
            overlap_atoms = sorted(a for a, c in atom_counts.items() if c > 1)
            translator.audit_drops.append(
                {
                    "kind": "verb_clause",
                    "atoms": overlap_atoms,
                    "reason": "overlap_bailout",
                }
            )
            return set()
    ...
    def _emit_syntax_row_files(verb: str, names: list[str]) -> int:
        """
        Emit one verb file per ``(verb, particle, iobj_prep, arity)`` cell
        in ``typed_rules[verb]`` via the ``syntax_row.py.j2`` Jinja template.

        Substitutes for the legacy per-verb-atom dispatcher when ``verb``
        is in :data:`MIGRATED_VERBS`.  Each rendered file is the
        action-chain runner for one syntax row — see Phase 3 of the
        refactor plan and the template body for the exact chain shape.

        :param verb: Canonical verb atom (e.g. ``"PUT"``).
        :param names: Player-facing aliases (including synonyms) the
            file should register in its shebang.
        :returns: Number of files written.
        """
        rules_for_verb = typed_rules.get(verb, [])
        if not rules_for_verb:
            return 0
        ...
        for rule in rules_for_verb:
            ...
            _write_unique(target_dir, f"{stem}.py", body)
            written += 1
        return written

    def _emit_v_routine_helper(routine: ZilRoutine) -> None:
        """
        Emit a passive V-* helper under ``verbs/thing/v_routines/``.

        Skipped for non-V-* routines and for routines whose body
        collapses to empty after translation.  The helper has shebang
        ``--on "Thing" --dspec none`` — the parser never dispatches it
        (``--dspec none`` requires no dobj at parse time, and our
        helpers are only ever called programmatically from the
        syntax-row action-chain runner introduced in Phase 3).

        Player-verb literals from the body (``the_player_verb == 'X'``
        and ``the_player_verb in ['X', 'Y']``) are added to the shebang's
        ``names`` list to satisfy the
        :func:`test_player_verb_alignment` ratchet — every literal the
        body checks must appear in the shebang.  Since the helper's
        ``dspec`` is ``none``, the extra names never affect parser
        dispatch (a verb that matches ``--dspec none`` requires the
        player to have typed no dobj — yet a V-* helper always carries
        a player-typed dobj).

        Body is produced from a *fresh* translator so the per-VERB? /
        per-M-clause split state from the legacy emission path doesn't
        leak in and shrink the helper to a residual.

        :param routine: A V-* action routine.
        """
        if not routine.name.startswith("V-"):
            return
        snake = atom_to_snake(routine.name[2:])
        if not snake:
            return
        helper_verb = f"v_{snake}"
        # Match the legacy emission's translator param set so the helper
        # body picks up the same pre-X prologue (``pre_x = "pre_drop";
        # if _.thing.invoke_verb(pre_x): return``), substrate display
        # names, and routine-to-verb mapping.  Without these, the helper
        # body diverges from the substrate body in subtle ways that
        # break behaviour parity at Phase 4 migration time.
        helper_translator = ZilTranslator(
            routine,
            object_atoms=object_atoms,
            routine_atoms=routine_atoms,
            ambiguous_object_atoms=ambiguous_object_atoms,
        ...
        body = helper_translator.translate()
        if not body.strip():
            return  # routine translates to empty — nothing to call
        # Extend the helper-verb list with every player-verb literal the
        # body checks so the alignment ratchet passes.  Sorted for
        # deterministic emission.
        body_literals = body_player_verb_literals(body)
        names = [helper_verb] + sorted(body_literals - {helper_verb})
        names_field = " ".join(names)
        # Rewrite the auto-emitted shebang.  The legacy shebang carries
        # parser-dispatch directives (``--dspec this``) the helper must
        # not inherit, since the syntax-row runner calls it programmatically.
        body = re.sub(
            r"^#!moo verb [^\n]*",
            f'#!moo verb {names_field} --on "Thing" --dspec none',
            body,
            count=1,
        )
        target_dir = _ensure_dir(verbs_dir / "thing" / "v_routines")
        _write_unique(target_dir, f"{helper_verb}.py", body)
    ...
    def _emit_routine(translator: ZilTranslator, target: Path, _name: str) -> None:
        ...
        action_owner = translator.action_owner

        # Phase 7d: emit one combined file per action_owner routine
        # (preturnfunc.py + turnfunc.py + … collapse into forest_room.py
        # with the routine atom + every M-/F-clause role name as
        # shebang aliases).  ``do_command``'s ``loc.invoke_verb(
        # "preturnfunc", "M-BEG", …)`` still resolves via the alias,
        # and the body's ``if rarg == "M-BEG":`` branch fires.
        # ``translate_combined_clauses`` flips on a parser-None safe
        # hoist for the duration of this emission so daemon-invoked
        # branches (M-ENTER from ``schedule_realtime``) don't crash.
        # Orphan routines (no action_owner) keep per-clause emission.
        if action_owner and (translator.has_m_dispatch() or translator.has_f_dispatch()):
            # Hand-written M-/F-clause role files (e.g. ``living_room/turnfunc.py``)
            # override the auto-translated body for THAT clause.  Pass the set
            # of clause constants whose role file is already on disk so
            # ``translate_combined_clauses`` drops them — both the body branch
            # AND the shebang alias — leaving the hand-written file as the
            # sole verb registering that role on the owner.
            skip_constants: set[str] = set()
            for const, role in M_TO_VERB.items():
                if (target / f"{role}.py").exists():
                    skip_constants.add(const)
            combined_code = translator.translate_combined_clauses(skip_constants=skip_constants)
            if combined_code:
                _write_unique(target, _filename_from_shebang(combined_code), combined_code)
                m_clause_count += 1
            for const in translator.m_constants_found():
                if const not in skip_constants:
                    audit.record_m_clause(_name, const, "combined")
            for const in translator.f_constants_found():
                if const not in skip_constants:
                    audit.record_f_clause(_name, const, "combined")
        # Phase 8 (scoped): the orphan-M/F per-clause emission fallback
        # for routines WITHOUT action_owner that have M-/F-dispatch is
        # dead code in both zork1 and hhg (every M-/F-dispatching
        # routine is owned by a room or object).  Removed; if a future
        # game surfaces a non-owned dispatcher, the audit will flag the
        # routine and translate_combined_clauses can be retrofitted to
        # handle it.

        clause_split_verbs: set[str] = set()
        if translator.has_verb_dispatch():
            # Phase 7 OBJECT-FUNCTION combined cut: for action_owner
            # objects (non-rooms), emit a single combined callback
            # invoked exclusively via ``dispatch_object_function``.
            # Replaces the per-VERB? split files entirely.  Rooms keep
            # using ``_emit_verb_clauses`` because their VERB? clauses
            # are typically room-scenery handlers that the parser
            # dispatches directly (e.g. ``examine carpet`` in the
            # bedroom — there's no ``carpet`` object, only the room
            # acting on the player's typed scenery word).
            if action_owner and not action_owner[1]:
                combined_object_code = translator.translate_object_function_combined()
                if combined_object_code:
                    _write_unique(
                        target,
                        _filename_from_shebang(combined_object_code),
                        combined_object_code,
                    )
                    # Mark every VERB? atom this callback handles as "covered"
                    # so the residual ``translate()`` skips them.
                    clause_split_verbs = set(translator._verbs_handled)  # pylint: disable=protected-access
                    # Also short-circuit translate()'s top-level VERB? prune
                    # (``_clause_split_emitted=True`` reflects "yes, we have
                    # an alternative dispatch path; don't bother emitting the
                    # god-residual").
                    translator._clause_split_emitted = True  # pylint: disable=protected-access
                else:
                    # Combined callback was empty (all clauses no-op or
                    # routine had no action_owner — fall back to legacy split.
    ...
    # Cross-V-routine duplicate alias dedup: ZIL syntax-tables can map a
    # single player verb (e.g. ``LIGHT``) to multiple V-routines via
    # different arities — ``LIGHT OBJECT = V-LAMP-ON`` and ``LIGHT OBJECT
    # WITH OBJECT = V-BURN``.  Both substrates would register ``light``
    # as a name on Thing with ``--dspec this``, and any parser lookup for
    # ``light`` on a dobj that inherits from Thing raises
    # AmbiguousVerbError.  The companion ``actor/dispatchers/light.py``
    # is the proper parser entry point — strip the shared alias from
    # every substrate that isn't the canonical V-{alias} owner so
    # ``get_verb("light")`` on Thing finds at most one substrate.
    _alias_owners: dict[str, set[str]] = {}
    for v_routine, bucket in _ROUTINE_TO_VERBS.items():
        primary_name = v_routine[2:].lower() if v_routine.startswith("V-") else v_routine.lower()
        for alias in bucket:
            if alias == primary_name:
                continue
            _alias_owners.setdefault(alias, set()).add(v_routine)
    _shared_aliases = {alias for alias, owners in _alias_owners.items() if len(owners) > 1}
    if _shared_aliases:
        for v_routine, bucket in list(_ROUTINE_TO_VERBS.items()):
            primary_name = v_routine[2:].lower() if v_routine.startswith("V-") else v_routine.lower()
            _ROUTINE_TO_VERBS[v_routine] = [
                alias for alias in bucket if alias == primary_name or alias not in _shared_aliases
            ]
    ...
    for name, routine in routines.items():
        if name in _SKIP_ROUTINES:
            audit.record_skipped(name, reason="_SKIP_ROUTINES")
            continue
        translator = ZilTranslator(
            routine,
            object_atoms=object_atoms,
        ...
        # Phase 4 Option A: skip the legacy substrate emission for
        # V-routines whose canonical verb is migrated.  The new
        # syntax_rows/<verb>.py file becomes the sole parser entry
        # point; without this skip, the legacy substrate has a lower
        # verb_id (loaded first by filesystem-walk order) and wins
        # parser tiebreak over the runner, leaving the action-chain
        # hooks dead.
        _migrated_substrate = (
            name.startswith("V-")
            and name[2:].lower().replace("-", "_") in MIGRATED_VERBS
            and action_owners.get(name) is None  # action-owned routines (OBJECT-FCN) handled in Phase 7
        )
        if not _migrated_substrate:
            _emit_routine(translator, target, name)

        # Phase 2 scaffolding: every V-* routine ALSO lands as a
        # passive helper under ``verbs/thing/v_routines/``.  The
        # syntax-row runner introduced in Phase 3 invokes these as
        # ``_.thing.v_<name>(...)``; until then they exist alongside
        # the legacy substrate-verb emission and are dispatch-inert
        # (``--dspec none``).  Phase 7 deletes the legacy path.
        _emit_v_routine_helper(routine)

        # Coverage audit: record the routine as emitted with its
        # action_owner.  M-/F-clause modes were recorded inside
        # ``_emit_routine`` already.  Drops captured on the translator
        # during emission are folded in now.
        audit.record_emitted(name, action_owner=action_owners.get(name))
        for drop in translator.audit_drops:
            audit.add_drop(name, **drop)
    ...
    syntax_count = 0
    for verb, rules in syntax_dict.items():
        # Per-syntax-row migration gate (Phase 3 of the refactor plan).
        # When the verb's lowercase atom is in :data:`MIGRATED_VERBS`,
        # emit one file per (particle, iobj_prep, arity) cell via the
        ...
        if _migration_forms & MIGRATED_VERBS:
            ...
            _deduped = list(dict.fromkeys(_names))
            syntax_count += _emit_syntax_row_files(verb, _deduped)
            continue
    ...
    # Phase 4 Option A: substrate verbs for migrated player verbs go
    # dispatch-inert so the new syntax-row runner wins parser dispatch.
    # The substrate body stays unchanged and is still invokable
    # programmatically via ``_.thing.<substrate_verb>()`` (or the actor
    # equivalent) from the runner — only the parser filter changes.
    #
    # Scope: every file under verbs/ EXCEPT the new syntax_rows/ tree
    # (those are the runners themselves) and the actor/dispatchers/
    # tree (those are gated off above for migrated verbs).  This
    # catches verbs/thing/substrate_verbs/<verb>.py AND legacy
    # actor/<verb>.py emissions (e.g. verbs/actor/inventory.py).
    if MIGRATED_VERBS:
        _dspec_re = re.compile(r"^(#!moo verb [^\n]*?\s+--dspec\s+)(?:this|either)(\b[^\n]*)", re.MULTILINE)
        _shebang_head = re.compile(r"^#!moo verb\s+([^\n]+?)(\s+--on\b|\s+--dspec\b|\s+--ispec\b|\n)")
        _excluded_subtrees = {
            (verbs_dir / "syntax_rows").resolve(),
            (verbs_dir / "actor" / "dispatchers").resolve(),
        }
        for path in sorted(verbs_dir.rglob("*.py")):
            if any(parent in _excluded_subtrees for parent in path.resolve().parents):
                continue
            text = path.read_text(encoding="utf-8")
            m = _shebang_head.match(text)
            if not m:
                continue
            names = m.group(1).split()
            # Only mute when every shebang name is migrated — per-object
            # handlers (e.g. kitchen_window/open.py with ``open close shut``)
            # carry non-migrated verbs that must still parse-match.
            if not set(names).issubset(MIGRATED_VERBS):
                continue
            new_text, count = _dspec_re.subn(r"\1none\2", text, count=1)
            if count:
                path.write_text(new_text, encoding="utf-8")

    coverage_path = audit.write(output_dir)
    print(
    ...
    print(f"Coverage:  {coverage_path}  ({audit.to_dict()['summary']['drops']} drops)")
