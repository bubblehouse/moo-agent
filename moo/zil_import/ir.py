...
class ZilSyntaxRule:
    """
    One ``<SYNTAX …>`` entry from a ZIL action table.

    Captures the ``(verb, particle, iobj_prep, arity)`` cell that the
    refactored generator can render as a syntax-row verb file.  See
    :doc:`/reference/zil-importer` for the emission model.

    :ivar verb: Canonical verb atom (e.g. ``"PUT"`` or ``"TURN"``).
    :ivar arity: Number of ``OBJECT`` slots — 0, 1, or 2.
    :ivar v_routine: Action routine name (e.g. ``"V-PUT-IN"``).
    :ivar particle: Word between the verb and the first ``OBJECT``
        (e.g. ``"OFF"`` in ``<SYNTAX TURN OFF OBJECT = V-LAMP-OFF>``).
        ``None`` when the rule is bare or only carries an iobj prep.
    :ivar iobj_prep: Preposition between the two ``OBJECT`` slots in a
        two-arity rule (e.g. ``"IN"`` in ``<SYNTAX PUT OBJECT IN OBJECT
        = V-PUT-IN>``).  ``None`` for arity < 2.
    """
