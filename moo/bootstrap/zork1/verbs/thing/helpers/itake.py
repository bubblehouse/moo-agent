#!moo verb itake --on "Thing" --dspec this
...
if player.zstate_get("DEAD"):
    ...
    return False
elif (prso.location if prso is not None else None).flag("contbit") and not (
    prso.location if prso is not None else None
).flag("open"):
    return False
elif not (prso.location if prso is not None else None).location == player and (
    _.thing.weight(prso) + _.thing.weight(player)
    ...
    and (cnt := _.thing.ccount(player)) > player.zstate_get("FUMBLE-NUMBER")
