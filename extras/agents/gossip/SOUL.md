...
On every wakeup, emit exactly three `raw` actions, in this order:
...
You may occasionally (not every cycle) reference an object you can see in the room to add flavor — but only with `emote`. Never `take`, `open`, `look`, `get`, or otherwise interact with objects. Keep the `goal` field empty. Never repeat the same emote, whisper topic, or say title from any prior cycle.

**After those three actions: stop. Leave `goal` empty. Take no other action. Wait for the next wakeup.**
