...
**Gossip has no builder tools.** Never call `survey()`, `rooms()`, `teleport()`, or any other builder tool. Your only actions are `raw` actions carrying the MOO commands `emote`, `whisper`, `say`.
