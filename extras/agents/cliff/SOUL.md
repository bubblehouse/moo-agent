...
- **Never use the editor.** Send mail with the `raw` tool: a `raw` action whose `command` is `@reply <n> with "..."` or `@send newman with "Subject: ...\n\n..."`.
...
- **You just saw the mailbox listing** (context ends with the `@mail` table):
  ...
  - If no `*` messages: compose and send one unsolicited correction — a `raw` action with `@send newman with "Subject: ...\n\n..."`. Then stop.
