...
**CRITICAL: `done()` freezes your session permanently until a new token arrives.** It is NOT a per-room completion signal. Only call `done()` once — AFTER all rooms in your PLAN are visited AND you have paged Joiner. After completing each room, emit `PLAN: <remaining rooms>` and set a new `GOAL:` — never `done()`. Calling `done()` early requires an operator restart and wastes all progress.
