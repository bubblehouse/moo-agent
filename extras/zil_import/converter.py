...
def extract_all(
    ...
    :param nodes: Top-level AST nodes from :func:`extras.zil_import.parser.parse`.
    ...
    for node in nodes:
        ...
        if head == "ROOM" and len(node) >= 2:
            ...
            #
            # Compound verbs like ``<SYNTAX TURN OFF OBJECT = V-LAMP-OFF>``
            # carry a particle (``OFF``) between the verb and OBJECT.
            # Side-record those in ``compound_verb_dict`` keyed by
            # ``(verb, particle)`` so the dispatcher generator can emit
            # particle-aware routing without changing the existing
            # syntax_dict tuple shape.
            ...
            # Bare and compound rules both go into syntax_dict so the
            # generator sees the full arity picture.  Compound rules
            # (those with particles) ALSO populate compound_verb_dict
            # — but the generator excludes them from _ROUTINE_TO_VERBS
            # so a compound like ``<SYNTAX EXAMINE IN OBJECT = V-LOOK-INSIDE>``
            # doesn't drag the bare-form ``examine`` alias into the
            # V-LOOK-INSIDE substrate's shebang (and conflict with
            # V-EXAMINE's substrate over the verb word ``examine``).
            syntax_dict.setdefault(verb, []).append((object_slots, v_routine))
            for particle in particles:
                compound_verb_dict.setdefault((verb, particle), v_routine)
            if not particles:
                # Track bare-form rules separately for _ROUTINE_TO_VERBS
                # population.  Storing alongside compound_verb_dict on the
                # caller side via a sentinel particle is brittle; this
                # mirror dict keeps the data model clean.
                bare_syntax_dict.setdefault(verb, []).append((object_slots, v_routine))
