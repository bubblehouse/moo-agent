...
Anything in `extras/zil_import/` is fair game. So is `moo/bootstrap/hhg/` (though it's generated output, so edits should land in `extras/zil_import/` and be regenerated).
