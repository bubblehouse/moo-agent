...
**DO NOT MODIFY `moo/` (OUTSIDE `moo/bootstrap/hhg/`) TO MAKE THE ZORK BOOTSTRAP WORK.**
...
Anything under `moo/` that is NOT `moo/bootstrap/hhg/` is core engine territory. That includes:
...
Anything in `moo/zil_import/` is fair game. So is `moo/bootstrap/hhg/` (though it's generated output, so edits should land in `moo/zil_import/` and be regenerated).
...
| Added zork1-specific banner | `moo/core/management/commands/moo_init.py` | `if bootstrap == "zork1":` — REMOVED. Banner should print from inside `moo/bootstrap/hhg/bootstrap.py`. |
...
## Boundary cases encountered (2026-06-02)

| Temptation | What I did instead |
| --- | --- |
| **"`Object.find()` doesn't descend into held containers — just teach the parser to flatten transparent-container contents."** While shaking down the improbability-drive win, the nested spare-drive parts (`switch`/`small plug`) wouldn't resolve from player input: `adv.find("switch")` → `[]` but `spare_drive.find("switch")` → the switch. Looked like a moo-core `Parser`/`Object.find` scope gap. | **Held the line — and it turned out NO core change was needed at all.** The game-side hook already exists: `do_command`'s `resolve_dobj_late` calls `_.peek_into(container, needle, 4)` over the player's inventory, descending into `open`/`transparent` held containers. The ONLY reason it wasn't firing was that the bare parser resolved "switch" → the **red button** (in the room, carrying a *stale* `switch` alias from an old bootstrap), so `parser.dobj` was non-None and `resolve_dobj_late` short-circuited. Clearing the stale alias (DB hygiene; current generation never emits it) let `peek_into` resolve the nested switch. **Lesson: when player input can't reach a nested object, check (a) a stale alias hijacking the dobj BEFORE the late hook runs, and (b) that the container has `open`/`transparent` — before ever suspecting `Object.find`.** The "isolated bare-`Parser` test shows dobj=None" was a red herring: the bare parser doesn't run `resolve_dobj_late`/`peek_into`; only real `do_command` dispatch does (see known-quirks "test as the Adventurer"). |
| **"The daemon clock has a same-tick cancel/self-re-queue race (I-ANNOUNCEMENT immortal) — fix the shared `queue.py` tick loop so a DISABLE always beats a same-pass self-ENABLE."** This is in `moo/zil_import/verbs/system/queue.py`, which IS in scope (not core) — so I tried it. | **Tried, REVERTED.** The change is Rule-Zero-legal but **broke the Vogon cascade** (`__survive_vogons__`). The lesson isn't Rule Zero, it's *blast radius*: `queue.py` is shared substrate that the whole daemon system depends on; the Vogon act relies on daemons cancelling/re-queuing each other in a specific same-tick order. Scope daemon-lifecycle fixes to the single offending daemon (an HHG override), never the shared clock — and always smoke-verify through the dependent content. See BUGS.md (Vogon intercom). |
...
4. Could this be done by adding a verb on a generated zork1 class (Zork Thing, Zork Room, etc.) — which lives in `moo/bootstrap/hhg/` and is therefore allowed?
