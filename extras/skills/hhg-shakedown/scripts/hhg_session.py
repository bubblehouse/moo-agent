#!/usr/bin/env python3
"""
Long-lived MooSSH session harness for the hhg-shakedown skill.

Holds a single SSH connection to ``hhg.local`` open for the duration of a
...
Subcommands::
    ...
    hhg_session.py start [--reset] [--user U] [--password P]
    ...
    hhg_session.py send <command>
        Send one HHG (or shell) command to the running session.  Returns
        immediately; output is appended to the log.

    hhg_session.py read [--tail N]
        Print the session log (or the last N lines).

    hhg_session.py since <marker>
...
# moo-agent/extras/skills/.  SCRIPT_DIR is .../hhg-shakedown/scripts/,
...
FIFO_PATH = Path("/tmp/hhg-shakedown.fifo")
LOG_PATH = Path("/tmp/hhg-shakedown.log")
PID_PATH = Path("/tmp/hhg-shakedown.pid")

DEFAULT_USER = "phil+hhg.local"
...
def cmd_start(args: argparse.Namespace) -> int:
    ...
    if args.reset:
        ...
        print(f"[{_ts()}] resetting world state on hhg.local...", flush=True)
        ...
        try:
            result = subprocess.run(
                [
                    ...
                    "sh",
                    "-c",
                    "/usr/app/bin/python /usr/app/src/manage.py moo_init --bootstrap hhg --sync --hostname hhg.local",
                ],
                capture_output=True,
                text=True,
                timeout=120,
    ...
    log_fp.write(f"[{_ts()}] starting hhg-shakedown session as {args.user}\n")
    log_fp.flush()
    ...
    try:
        ...
        return 1
    log_fp.write(f"[{_ts()}] connected to universe: hhg.local\n")
    log_fp.flush()
...
def main() -> int:
    parser = argparse.ArgumentParser(prog="hhg_session")
