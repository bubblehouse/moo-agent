...
| Tinker uses `@move #N to #M` instead of `place #N on #M` after creating an object | Agent conflates physical relocation with spatial placement; `@move` is for changing an object's containing room, `place` sets on/under/behind metadata | Add to SOUL.md `## Placement`: explicit "`place` is NOT `@move`" note with one-line explanation of the difference |
