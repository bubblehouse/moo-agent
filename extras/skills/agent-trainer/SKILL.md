...
| Tinker uses `@move #N to #M` instead of `place #N on #M` after creating an object | `place` was not a registered tool; LLM grabbed the nearest familiar verb (`@move`) | Now fixed: `place`, `open`, `close`, `put`, `take`, `drop` are all registered tools in `BUILDER_TOOLS`. Add them to the agent's `## Tools` list and use tool-call syntax in SOUL.md examples. |
| Quartermaster teleports to The Agency and aliases/describes existing objects | `divine()` returns The Agency (#23); LLM surveys it, finds existing objects (e.g. the leather satchel), and treats them as test targets — operating on objects it didn't create | Add to SOUL.patch.md: (1) filter #23 from PLAN — never put The Agency in your plan; (2) call `divine()` from wherever you are — no teleport to The Agency first; (3) never operate on objects found by `survey()` that you did not just create in this session |
| Agent calls a tool not in its `## Tools` list (e.g. Quartermaster calls `read_board`) | LLM generalizes from other agents' SOULs in its training context; sees `read_board` used by Joiner/Tinker and assumes it applies universally | Add explicit "X is not one of your tools — call Y instead" rule to SOUL.patch.md; list the correct alternative (for Quartermaster: `divine()` is the only room source) |
| Agent calls `read_board` or `write_book` from outside The Agency — returns empty or fails | LLM omits the required `teleport(destination="The Agency")` step; the dispatch board and survey book are physical objects located in The Agency and are not accessible from other rooms | Room Traversal step 1 in all builder SOULs (Joiner, Tinker, Harbinger, Stocker) must be `teleport(destination="The Agency")` before any `read_board` call. Added CRITICAL block to `baseline.md` Coordination Objects section: "Neither object is readable from any other location." |
