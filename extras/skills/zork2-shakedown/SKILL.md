---
name: zork2-shakedown
description: End-to-end ZIL→DjangoMOO debugging skill for Zork II (The Wizard of Frobozz). Drive zork2.local through MooSSH to find translator/server bugs, OR pick a known failure and fix it inside moo/zil_import/ (translator/generator/SDK/config). Use when the user asks to shake down Zork II, raise its smoke pass count, debug a zork2 failure, or close a translation gap for Zork II. Read ../_shared/references/rule-zero.md before any edit under moo/.
compatibility: Designed for Claude Code. Requires the django-moo repository, a running docker-compose stack, the moo-agent extension installed (provides moo.bootstrap.zork2), Zork II ZIL source at /Users/philchristensen/Workspace/zork2/, and the zork2.local Site initialised.
---

# Zork II Shakedown

The full Zork II debugging loop: **find** bugs by playing the canonical
world, then **fix** them inside `moo/zil_import/` and verify with the smoke
harness. One skill, two modes. Patterned after `zork-shakedown` — Zork II is
...
- **moo-agent** is the working directory. The Zork II dataset
  (`moo/bootstrap/zork2/`, gitignored generated output), the ZIL importer
  (`moo/zil_import/`), and this skill all live here.
- **Zork II ZIL source** is at `/Users/philchristensen/Workspace/zork2/`.
  Manifest is `zork2.zil` (`<INSERT-FILE>` pulls in `2dungeon.zil`,
  `2actions.zil`, and the shared `g*.zil` parser files). `ZORK-NUMBER` is 2.
...
`moo.bootstrap.zork2` resolves regardless of working directory.

## 🛑 RULE ZERO — READ BEFORE EVERY EDIT IN `moo/`

**DO NOT MODIFY `moo/` (OUTSIDE `moo/bootstrap/zork2/`) TO MAKE THE ZORK II BOOTSTRAP WORK.**
...
`game_config.py` (the `ZORK2_CONFIG` knobs), in `moo/zil_import/verbs/zork2/`
...
specifics belong in `game_config.py` / `verbs/zork2/`, never hardcoded into
the importer.
...
| `scripts/zork2_session.py` | Thin wrapper around the shared MooSSH harness. |
...
You are a Zork II player who happens to be a MOO engine debugger. Drive
...
extras/skills/zork2-shakedown/scripts/zork2_session.py send "look"
extras/skills/zork2-shakedown/scripts/zork2_session.py read --tail 30
extras/skills/zork2-shakedown/scripts/zork2_session.py since "look"
extras/skills/zork2-shakedown/scripts/zork2_session.py stop
```
...
### Canonical Zork II opener

Zork II begins in a barrow-like area near the entrance to the underground.
The first session should map the actual opening room and exits from
`2dungeon.zil` and tick them into `references/coverage.md` — Zork II's map is
not Zork 1's, so don't assume the West-of-House sequence.
...
The smoke test (`moo/zil_import/scripts/zork2_smoke.py`) drives a canonical
...
4. Read [../_shared/references/smoke-workflow.md](../_shared/references/smoke-workflow.md) — the regen/sync/smoke loop (substitute `<slug>` = `zork2`).
...
2. **Locate the cause.** Most live in `moo/zil_import/translator/` (verb-body
   ...
   (`ZORK2_CONFIG` — NPC atom map, synonym/adjective 6-char truncations,
   exit-condition overrides), or `moo/zil_import/verbs/zork2/` (runtime shims).
3. **Edit only inside the allowed surface.** Can't fix without core? Stop and ask.
4. **Regen + sync** (see smoke-workflow.md):
   `uv run python -m moo.zil_import ~/Workspace/zork2/zork2.zil --game-config zork2 --output .../moo-agent/moo/bootstrap/zork2`
   then `moo_init --bootstrap zork2 --sync --hostname zork2.local`.
5. **Spot-test** with `zork2_spot.py`, then run the full `zork2_smoke.py`.
...
- `reference_site_pk_layout` — which Site PK is `zork2.local`.
